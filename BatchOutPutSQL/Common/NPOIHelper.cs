...
namespace Test2
{
    public class NPOIHelper
    {
        ...
        /// <param name="strHeaderText"></param>
        /// <param name="strFileName"></param>
        public static void TempExportByWeb(DataSet ds, string strHeaderText, string strFileName)
        {
            HttpContext curContext = HttpContext.Current;
            curContext.Response.ContentType = "application/vnd.ms-excel";
            curContext.Response.Charset = "";
            ...
            //  curContext.Response.AppendHeader("Content-Disposition", "attachment;filename=" +strFileName);
            curContext.Response.ContentEncoding = System.Text.Encoding.GetEncoding("utf-8");
            curContext.Response.BinaryWrite(TempExportDataSetToExcel(ds, strHeaderText).GetBuffer());
            curContext.Response.End();
        }
        ...
        private static MemoryStream TempExportDataSetToExcel(DataSet sourceDs, string sheetName)
        {
            HSSFWorkbook workbook = new HSSFWorkbook();
            ...
            for (int i = 0; i < sheetNames.Length; i++)
            {
                ...
                if (!string.IsNullOrEmpty(sheetName))
                {
                    ...
                    headerRow2.CreateCell(0).SetCellValue(sheetName);
                ...
                headerRow.CreateCell(0).SetCellValue("头1");
                headerRow.GetCell(0).CellStyle = headStyle;

                headerRow.CreateCell(1).SetCellValue("头1");
                headerRow.GetCell(1).CellStyle = headStyle;

                headerRow.CreateCell(2).SetCellValue("头1");
                headerRow.GetCell(2).CellStyle = headStyle;

                headerRow.CreateCell(3).SetCellValue("头1");
                headerRow.GetCell(3).CellStyle = headStyle;

                //设置宽度参考下面注释掉的SetColumnWidth
                ...
                foreach (DataRow row in sourceDs.Tables[i].Rows)
                {
                    ...
                    foreach (DataColumn column in sourceDs.Tables[i].Columns)
                    {
                        if (row[column].ToString().Contains("月份") && !IsFirst)
                        {
                            TempIndex = 0;
                            rowIndex++;
                            dataRow = sheet.CreateRow(rowIndex);
                            dataRow.CreateCell(TempIndex).SetCellValue(row[column].ToString());
                        }
                        else
                        {
                            IsFirst = false;
